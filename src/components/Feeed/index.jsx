...
function Feed({ photoUrl, username, showCreatePost, entityId, entityType, showDeleteButton, admin }) {
  ...
  const handleLikes = async (entityId) => {
    try {
      const response = await axios.get(`http://34.229.93.25:5000/${entityType}/${entityId}`);
  ...
  const refreshComments = async (postId) => {
    try {
      const response = await axios.get(`http://34.229.93.25:5000/${entityType}/${postId}`);
      const updatedPost = response.data;
      setPosts((prevPosts) => {
        return prevPosts.map((post) => {
          if (post._id === postId) {
  ...
  const getPosts = async (page) => {
    ...
    try {
      const response = await axios.get(
        `http://34.229.93.25:5000/${entityType}?page=${page}&size=${LIMIT}`
      );
